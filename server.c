...
char* indexes(const char* path)
{
    ...
    Path =  realloc(Path, strlen(path) + 12);
    strcpy(Path, path);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    while((bytes = fread(Buffer, 1, BYTES, file)) != 0)
    {
...
const char* lookup(const char* path)
{
    //point to the extension of file
    const char* h = strrchr(path,'.');
    
    if(strcasecmp(h,".css") == 0)
        return("text/css");
    else if(strcasecmp(h,".html") == 0)
        return("text/html");
    else if(strcasecmp(h,".gif") == 0)
        return("image/gif");
    else if(strcasecmp(h,".ico") == 0)
        return("image/x-icon");
    else if(strcasecmp(h,".jpg") == 0)
        return("image/jpeg");
    else if(strcasecmp(h,".js") == 0)
        return("text/javascript");
    else if(strcasecmp(h,".php") == 0)
        return("text/x-php");
    else if(strcasecmp(h,".png") == 0)
        return("image/png");
    else
        return NULL;
}
...
{
    int spacePos[2], spaceCount;
    spacePos[0] = spacePos[1] = -1;
    spaceCount = 0;
    ...
    //request method
    char method[spacePos[0]];
    strncpy(method, line, spacePos[0] -1);
    method[spacePos[0] -1] = '\0';
    
    char requestTarget[spacePos[1] - spacePos[0]];
    strncpy(requestTarget, line + spacePos[0], spacePos[1] - spacePos[0] - 1);
    requestTarget[spacePos[1] - spacePos[0] - 1] = '\0';
    
    const char* httpVersion = line + spacePos[1];
    
    if((strstr(httpVersion, "\r\n") == NULL) || (strchr(requestTarget, '\"') != NULL))
    {
        error(400);
        return false;
    }
    ...
    if(strncmp(httpVersion, "HTTP/1.1", strlen(httpVersion) - 2) != 0)
    {
        error(505);
        return false;
    }
    
    char* Query = strrchr(requestTarget, '?');
    ...
    {
        strncpy(query, Query + 1, strlen(Query) - 1);
        query[strlen(Query) - 1] = '\0';
        
        strncpy(abs_path, requestTarget, strlen(requestTarget) - strlen(Query));
        abs_path[strlen(requestTarget)-strlen(Query)] = '\0';
